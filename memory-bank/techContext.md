...
- Zsh completion (`completion.zsh`) mirrors bash: `_get_repo_dir` and `_ai_rizz_list_rule_names` must stay algorithmically identical to `completion.bash` (top-level `rules/*.mdc`, `rules/*.md`, standalone `rules/<name>/SKILL.md` only — same as `cmd_list`). `_ai_rizz` dispatches on `words[CURRENT-1]` like bash `prev`. Installer is [`install-zsh-completion.sh`](../install-zsh-completion.sh) (POSIX sh); `make install` writes a shell's fence only if that shell is on `PATH`. Tests: [`tests/unit/test_zsh_completion.test.sh`](../tests/unit/test_zsh_completion.test.sh) (requires `zsh` on `PATH`; fail, do not skip).
