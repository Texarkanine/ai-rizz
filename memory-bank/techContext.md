...
- Zsh completion (`completion.zsh`) mirrors bash: `_get_repo_dir` and `_ai_rizz_list_rule_names` must stay algorithmically identical to `completion.bash` (top-level `rules/*.mdc`, `rules/*.md`, standalone `rules/<name>/SKILL.md` only — same as `cmd_list`). `_ai_rizz` dispatches on `words[CURRENT-1]` like bash `prev`. Installer fence runs `compinit -C` before sourcing; `completion.zsh` also bootstraps `compinit` when `compdef` is missing. Tests: [`tests/unit/test_zsh_completion.test.sh`](../tests/unit/test_zsh_completion.test.sh) (requires `zsh` on `PATH`; fail, do not skip).
