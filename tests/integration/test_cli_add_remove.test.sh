...
# Test: ai-rizz add rule <rule> --global migrates from local
# Expected: Rule should be removed from local manifest and local directory
test_add_rule_global_migrates_from_local() {
    # Initialize both local and global modes
    run_ai_rizz init "file://$MOCK_REPO_DIR" -d .cursor/rules --local
    assertEquals "Local init should succeed" 0 $?
    run_ai_rizz init "file://$MOCK_REPO_DIR" --global
    assertEquals "Global init should succeed" 0 $?
    
    # Add rule locally first
    run_ai_rizz add rule rule1 --local
    assertEquals "Local add should succeed" 0 $?
    assert_manifest_contains "ai-rizz.local.skbd" "rules/rule1.mdc"
    assert_rule_deployed ".cursor/rules/local" "rule1"
    
    # Now add the same rule globally
    run_ai_rizz add rule rule1 --global
    assertEquals "Global add should succeed" 0 $?
    
    # Rule should be in global manifest
    assert_manifest_contains "$HOME/ai-rizz.skbd" "rules/rule1.mdc"
    
    # Rule should be removed from local manifest
    assert_manifest_not_contains "ai-rizz.local.skbd" "rules/rule1.mdc"
    
    # Rule should be removed from local directory after sync
    assert_rule_not_deployed ".cursor/rules/local" "rule1"
