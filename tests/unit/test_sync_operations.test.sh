...
test_sync_all_initialized_modes() {
    ...
    rm -f "$TEST_TARGET_DIR/$TEST_SHARED_DIR/rule3.mdc"
...
test_sync_dual_mode_with_rulesets() {
    ...
    rm -rf "$TEST_TARGET_DIR/$TEST_LOCAL_DIR"
    rm -rf "$TEST_TARGET_DIR/$TEST_SHARED_DIR"
...
test_sync_commit_mode_only() {
    ...
    # Delete file to test sync
    rm -f "$TEST_TARGET_DIR/$TEST_SHARED_DIR/rule1.mdc"
    ...
    assertFalse "Local directory should not be created" "[ -d '$TEST_TARGET_DIR/$TEST_LOCAL_DIR' ]"
}
...
test_sync_empty_manifest_creates_directories() {
    ...
    assertTrue "Local directory should be created" "[ -d '$TEST_TARGET_DIR/$TEST_LOCAL_DIR' ]"
}
...
test_sync_triggers_after_remove() {
    ...
    assert_file_not_exists "$TEST_TARGET_DIR/$TEST_LOCAL_DIR/rule1.mdc"
    assert_file_exists "$TEST_TARGET_DIR/$TEST_SHARED_DIR/rule3.mdc"  # Should remain
}
...
test_sync_handles_missing_target_directories() {
    ...
    assertTrue "Target directory should be recreated" "[ -d '$TEST_TARGET_DIR' ]"
    assertTrue "Local directory should be recreated" "[ -d '$TEST_TARGET_DIR/$TEST_LOCAL_DIR' ]"
    assertTrue "Shared directory should be recreated" "[ -d '$TEST_TARGET_DIR/$TEST_SHARED_DIR' ]"
    assert_file_exists "$TEST_TARGET_DIR/$TEST_LOCAL_DIR/rule1.mdc"
    assert_file_exists "$TEST_TARGET_DIR/$TEST_SHARED_DIR/rule3.mdc"
}
...
test_sync_handles_partial_file_corruption() {
    ...
    echo "corrupted content" > "$TEST_TARGET_DIR/$TEST_LOCAL_DIR/rule1.mdc"
    
    # Test: Sync should restore correct content
    cmd_sync
    
    # Expected: File should be restored to original content
    content=$(cat "$TEST_TARGET_DIR/$TEST_LOCAL_DIR/rule1.mdc")
...
test_sync_with_custom_target_directory() {
    ...
    rm -f "$custom_dir/$TEST_LOCAL_DIR/rule1.mdc"
